...
def B_RDAQ(flat_grad, side_inf, diagonal, Bnd):
    ...
    #Nonuniform_Level
    temp_M_1=np.digitize(np.absolute(rotated_norm_grad), M)
    temp_M_2=np.digitize(np.absolute(rotated_norm_side), M)
    temp_M=np.maximum(temp_M_1, temp_M_2)
    ...
    for w in range(B):
        U_random = np.random.rand(d)   #### Indeoendent shared randomness per repetition
...
for n in [5,10,20,30, 40, 50]:
	...
	#sigma_range = [0.000078125*(2**(i-4)) for i in range(8)]  ### Uncomment it for 6 and 8  bit comparisons
	...
	for sig in sigma_range:
		...
		print('RMSE over iterations: RDAQ', MSE_B_RDAQ_I)
	...
	np.savetxt('n='+str(n)+"_RMSE_B_RDAQ_"+str(int(4*np.log2(B+1))+2)+"_"+str(d)+".dat", MSE_B_RDAQ, delimiter =", ", fmt ='% s')
